...
const IMG_BASE = "./featured/";

function priceHTML(c){
  if (c.priceNew === 0) {
    return `<span class="old">$${c.priceOld.toFixed(1)}</span><span class="free">Free</span>`;
  }
...
function courseCardTpl(c){
  ...
  <div class="course-card" data-id="${c.id}">
    ...
    <div class="card-body">
      ...
      <div class="card-meta">
        <div class="meta-item">
          <img src="./featured/icon1.png" alt="duration">
          <span>${c.durationWeeks} Weeks</span>
        </div>
        <div class="meta-item">
          <img src="./featured/icon2.png" alt="students">
...
function applyFilters(){
  ...
  if (state.price.size){
    list = list.filter(c => {
      ...
      if (state.price.has("Free") && isFree) return true;
      if (state.price.has("Paid") && !isFree) return true;
      return false;
    });
  }
...
function renderCourses(){
  ...
  gridEl.innerHTML = slice.map(courseCardTpl).join("") || `
  ...
  for (let p=1; p<=pages; p++) pagerEl.appendChild(btn(String(p), p, false, p===state.page));
  pagerEl.appendChild(btn("›", Math.min(pages, state.page+1), state.page===pages));
}
...
const catGroup   = $$(".filter-group").find(g => g.querySelector("h3")?.textContent.includes("Course Category"));
const insGroup   = $$(".filter-group").find(g => g.querySelector("h3")?.textContent.includes("Instructors"));
const priceGroup = $$(".filter-group").find(g => g.querySelector("h3")?.textContent.trim()==="Price");
const levelGroup = $$(".filter-group").find(g => g.querySelector("h3")?.textContent.trim()==="Level");
const reviewGroup= $$(".filter-group").find(g => g.querySelector("h3")?.textContent.trim()==="Review");
